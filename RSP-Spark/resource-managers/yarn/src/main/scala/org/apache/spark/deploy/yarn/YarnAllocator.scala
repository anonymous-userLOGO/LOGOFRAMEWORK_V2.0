...
private[yarn] class YarnAllocator(
   ...
   * Visible for testing.
   */
 def getRunningHostList(args: String): Array[String] = {
    ...
    for (word <- lines){
      tag = tag+1
      if(tag > 2){
        val line = word.replaceAll(" ","")
        var flag = true
        for (txt<-line.split(":") if flag){
          string = string  + txt + "-"
          flag = false
        }
      }
  ...
  def updateResourceRequests(): Unit = {
    ...
    if( conf.get("spark.rsp") == "true" ){
      val cmd = "yarn node -list"
      val result = cmd.!!
      val hostList = getRunningHostList(result)
      logInfo(s"i me am   "+ hostList.mkString("Array(",",",")"))
      var nodeToCreateExecutor=(hostList.toSet--allocatedHostToContainersMap.keySet.toSet).toArray
      
      val missing = nodeToCreateExecutor.size

      if (missing >0) {
        nodeToCreateExecutor=nodeToCreateExecutor.take(1)
        nodeToCreateExecutor.foreach{
          case node =>
            allocatedHostToContainersMap.getOrElseUpdate(node , new HashSet[ContainerId])
            val pendingList = getPendingAtLocation(node)
            if (pendingList.isEmpty){
              amClient.addContainerRequest(createContainerRequest(resource, nodeToCreateExecutor, null))
            }
          case _ =>
        }
      }
  ...
  private def createContainerRequest(
      ...
      racks: Array[String]): ContainerRequest = {
        val conf = new SparkConf()
        if( conf.get("spark.rsp") == "true" ){
           new ContainerRequest(resource, nodes, racks, RM_REQUEST_PRIORITY, false, labelExpression.orNull)
        }
        else{
